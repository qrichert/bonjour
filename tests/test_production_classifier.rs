...
fn cli_greeting_and_json_contracts_are_exact_with_runtime_data() {
    ...
    assert_eq!(relational["best_candidate"], "Arthur");
    assert_eq!(relational["greeting_name"], "Arthur");
    assert!(relational.get("selected_candidate").is_none());
    ...
    assert_eq!(relational["decision"]["c5"]["passed"], false);
...
#[cfg(not(feature = "standalone"))]
fn assert_c5_rule_json(rule: &serde_json::Value, passed: bool) {
    let rule = rule.as_object().unwrap();
    assert_eq!(rule.len(), 14);
