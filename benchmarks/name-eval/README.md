...
`Olivier Sieffert` selected `Olivier` at the unchanged C3.1 score, and
`Baris Aras` selected `Baris`, but neither relational path passed. The
generated `c4_qualitative_diagnostics.json` records the emission source,
winner features, provenance, vetoes, and each branch condition.
