...
mod surname_complement_validation;
mod surname_index_selection;
...
use calibration_frontier::{
    ...
    run_complement_diagnostic, run_morphology_diagnostic, run_ordering_diagnostic,
    run_sealed_c4_c5_comparison,
};
use classifier::{
...
struct Arguments {
    ...
    complement_surname_counts: Option<PathBuf>,
    complement_surname_manifest: Option<PathBuf>,
    surname_inventory: Option<PathBuf>,
    surname_key_directory: Option<PathBuf>,
    surname_name_totals: Option<PathBuf>,
    surname_selection_probes: Option<PathBuf>,
    surname_membership_directory: Option<PathBuf>,
    surname_member_keys: Option<PathBuf>,
    surname_verification_receipt: Option<PathBuf>,
...
fn parse_arguments_from(arguments: impl IntoIterator<Item = OsString>) -> Result<Arguments> {
    ...
    let mut complement_surname_counts = None;
    let mut complement_surname_manifest = None;
    let mut surname_inventory = None;
    let mut surname_key_directory = None;
    let mut surname_name_totals = None;
    let mut surname_selection_probes = None;
    let mut surname_membership_directory = None;
    let mut surname_member_keys = None;
    let mut surname_verification_receipt = None;
    ...
    for argument in arguments {
        ...
        } else if text == "--diagnose-ordering-evidence" {
            diagnose_ordering_evidence = true;
        } else if text == "--diagnose-capitalization-evidence" {
        ...
        } else if text == "--prepare-surname-index-selection" {
        ...
        } else if let Some(value) = text.strip_prefix("--complement-probes=") {
            complement_probes = Some(PathBuf::from(value));
        } else if let Some(value) = text.strip_prefix("--complement-surname-counts=") {
            complement_surname_counts = Some(PathBuf::from(value));
        } else if let Some(value) = text.strip_prefix("--complement-surname-manifest=") {
            complement_surname_manifest = Some(PathBuf::from(value));
        } else if let Some(value) = text.strip_prefix("--surname-inventory=") {
            surname_inventory = Some(PathBuf::from(value));
        } else if let Some(value) = text.strip_prefix("--surname-key-directory=") {
            surname_key_directory = Some(PathBuf::from(value));
        } else if let Some(value) = text.strip_prefix("--surname-name-totals=") {
            surname_name_totals = Some(PathBuf::from(value));
        } else if let Some(value) = text.strip_prefix("--surname-selection-probes=") {
            surname_selection_probes = Some(PathBuf::from(value));
        } else if let Some(value) = text.strip_prefix("--surname-membership-directory=") {
            surname_membership_directory = Some(PathBuf::from(value));
        } else if let Some(value) = text.strip_prefix("--surname-member-keys=") {
            surname_member_keys = Some(PathBuf::from(value));
        } else if let Some(value) = text.strip_prefix("--surname-verification-receipt=") {
            surname_verification_receipt = Some(PathBuf::from(value));
    ...
    if validate_frozen_surname_candidate_sha256
        .as_deref()
        .is_some_and(|digest| digest != surname_complement_validation::V8_SHA256)
    {
        return Err(
        ...
        || select_freeze_c5_operating_point
    {
        if positional.len() != 2 || sealed.is_some() || development_only || reference_threshold_set
        {
            ...
            } else if diagnose_complement_evidence {
                "--diagnose-complement-evidence"
            } else if diagnose_capitalization_evidence {
                "--diagnose-capitalization-evidence"
            } else if diagnose_ordering_evidence {
                "--diagnose-ordering-evidence"
    ...
    let valid_frozen_surname_files = if verify_frozen_surname_candidate {
        ...
        surname_membership_directory.is_some()
            ...
            && surname_name_totals.is_none()
            && surname_verification_receipt.is_some()
    } else {
        surname_membership_directory.is_none()
            && surname_member_keys.is_none()
            && surname_verification_receipt.is_none()
    };
    if !valid_frozen_surname_files {
        return Err("surname-candidate verification requires membership, member-key, and name-total paths; V8 validation requires membership and verification-receipt paths; these inputs are forbidden by other modes".into());
    }
    Ok(Arguments {
        artifact,
        clean_csv,
...
fn validate_spent_arguments(
    ...
    if eight_proxy_enabled {
        ...
        if actual != expected {
            return Err(format!(
                "seven-proxy surname-index modes require exactly the acknowledged V1-V7 digests; received {actual:?}"
            )
            .into());
        }
    } else if frontier_enabled {
        ...
        if actual != expected {
            return Err(format!(
                "five-proxy diagnostics require exactly the acknowledged V1/V2/V3/V4/V5 digests; received {actual:?}"
            )
            .into());
        }
    }
...
fn evaluate(arguments: &Arguments, output: &Path) -> Result<String> {
    ...
    if arguments.prepare_surname_index_selection {
        return prepare_surname_index_selection(
            output,
    ...
    if arguments.verify_frozen_surname_candidate {
        return verify_frozen_surname_candidate(
            output,
            arguments
                .surname_membership_directory
                .as_deref()
                .ok_or("surname-candidate verification is missing its membership directory")?,
            arguments
                .surname_member_keys
                .as_deref()
                .ok_or("surname-candidate verification is missing its member-key stream")?,
            arguments
                .surname_name_totals
                .as_deref()
    ...
    if arguments.validate_frozen_surname_candidate_sha256.is_some() {
        let holdout = frozen_holdout
            .as_ref()
            .ok_or("V8 surname-complement validation requires a frozen holdout")?;
        return run_surname_complement_validation(
            output,
            &corpus,
            holdout,
            arguments
                .surname_membership_directory
                .as_deref()
                .ok_or("V8 validation is missing its surname membership directory")?,
            arguments
                .surname_verification_receipt
                .as_deref()
    ...
    if arguments.select_surname_only_index {
        return run_surname_index_selection(
            output,
            &corpus,
            load_spent_holdouts(arguments)?,
            arguments
                .complement_surname_counts
                .as_deref()
                .ok_or("surname-index selection is missing targeted surname counts")?,
            arguments
                .complement_surname_manifest
                .as_deref()
                .ok_or("surname-index selection is missing its surname manifest")?,
            arguments
                .surname_inventory
                .as_deref()
                .ok_or("surname-index selection is missing its inventory")?,
            arguments
                .surname_key_directory
                .as_deref()
                .ok_or("surname-index selection is missing its key directory")?,
            arguments
                .surname_name_totals
                .as_deref()
                .ok_or("surname-index selection is missing name totals")?,
            arguments
                .surname_selection_probes
                .as_deref()
    ...
    if arguments.diagnose_complement_evidence {
        ...
        let probes = arguments
            .complement_probes
            .as_deref()
            .ok_or("complement diagnostic is missing its qualitative probes path")?;
        let surname_counts = arguments
            .complement_surname_counts
            .as_deref()
            .ok_or("complement diagnostic is missing its surname counts path")?;
        let surname_manifest = arguments
            .complement_surname_manifest
            .as_deref()
            .ok_or("complement diagnostic is missing its surname manifest path")?;
        return run_complement_diagnostic(
            output,
            &corpus,
            holdouts,
            &fixtures,
    ...
    if arguments.diagnose_morphology_evidence {
        let holdouts = load_spent_holdouts(arguments)?;
        let totals = arguments
            .morphology_name_totals
            .as_deref()
    ...
    if arguments.diagnose_capitalization_evidence {
        let holdouts = load_spent_holdouts(arguments)?;
        return run_capitalization_diagnostic(output, &corpus, holdouts, &fixtures);
    }
    if arguments.diagnose_ordering_evidence {
        let holdouts = load_spent_holdouts(arguments)?;
        return run_ordering_diagnostic(output, &corpus, holdouts, &fixtures);
    }
...
mod argument_tests {
    ...
    fn capitalization_diagnostic_arguments() -> Vec<String> {
        ...
        let mode = arguments
            ...
            .find(|argument| argument.as_str() == "--diagnose-c5-calibration-frontier")
            .unwrap();
        *mode = "--diagnose-capitalization-evidence".to_string();
        arguments
    }

    fn morphology_diagnostic_arguments() -> Vec<String> {
        let mut arguments = calibration_frontier_arguments();
        let mode = arguments
            .iter_mut()
            .find(|argument| argument.as_str() == "--diagnose-c5-calibration-frontier")
            .unwrap();
        *mode = "--diagnose-morphology-evidence".to_string();
        arguments.push("--morphology-name-totals=name-totals.csv".to_string());
        arguments
    }

    fn complement_diagnostic_arguments() -> Vec<String> {
        let mut arguments = calibration_frontier_arguments();
        let mode = arguments
            .iter_mut()
            .find(|argument| argument.as_str() == "--diagnose-c5-calibration-frontier")
            .unwrap();
        *mode = "--diagnose-complement-evidence".to_string();
        arguments.extend([
            "--complement-probes=probes.csv".to_string(),
            "--complement-surname-counts=surname-counts.csv".to_string(),
            "--complement-surname-manifest=surname-manifest.csv".to_string(),
        ]);
        arguments
    }

    fn surname_index_arguments(prepare: bool) -> Vec<String> {
        let mut arguments = if prepare {
            vec![
                "output".to_string(),
                "--prepare-surname-index-selection".to_string(),
                "--surname-selection-probes=probes.csv".to_string(),
            ]
        } else {
            vec![
                "artifact".to_string(),
                "output".to_string(),
                "--select-surname-only-index".to_string(),
                "--complement-surname-counts=surname-counts.csv".to_string(),
                "--complement-surname-manifest=surname-manifest.csv".to_string(),
                "--surname-inventory=inventory.csv".to_string(),
                "--surname-key-directory=keys".to_string(),
                "--surname-name-totals=name-totals.csv".to_string(),
                "--surname-selection-probes=probes.csv".to_string(),
            ]
        };
        for (version, digest) in [
            ("v1", surname_index_selection::V1_SHA256),
            ("v2", surname_index_selection::V2_SHA256),
            ("v3", surname_index_selection::V3_SHA256),
            ("v4", surname_index_selection::V4_SHA256),
            ("v5", surname_index_selection::V5_SHA256),
            ("v6", surname_index_selection::V6_SHA256),
            ("v7", surname_index_selection::V7_SHA256),
        ] {
            arguments.extend([
                format!("--spent-holdout={version}/sealed.csv"),
                format!("--spent-manifest={version}/sealed.manifest.csv"),
    ...
    fn surname_candidate_verification_arguments() -> Vec<String> {
        vec![
            "output".to_string(),
            "--verify-frozen-surname-candidate".to_string(),
            "--surname-membership-directory=membership".to_string(),
            "--surname-member-keys=members.txt".to_string(),
            "--surname-name-totals=name-totals.csv".to_string(),
        ]
    }

    fn v8_validation_arguments() -> Vec<String> {
        vec![
            "artifact".to_string(),
            "output".to_string(),
            format!(
            ...
            "--surname-membership-directory=membership".to_string(),
            "--surname-verification-receipt=artifact-validation.csv".to_string(),
            "--sealed=sealed.csv".to_string(),
            "--sealed-manifest=manifest.csv".to_string(),
        ]
    }
    ...
    #[test]
    fn ordering_diagnostic_requires_exactly_v1_through_v5() {
        let arguments = parse_owned(ordering_diagnostic_arguments()).unwrap();
        assert!(arguments.diagnose_ordering_evidence);
        assert!(!arguments.diagnose_c5_calibration_frontier);
        assert_eq!(arguments.clean_csv, None);
        assert_eq!(arguments.spent_holdouts.len(), 5);

        let mut incomplete = ordering_diagnostic_arguments();
        incomplete.truncate(incomplete.len() - 3);
        assert!(parse_owned(incomplete).is_err());

        let mut wrong_digest = ordering_diagnostic_arguments();
        let last = wrong_digest.len() - 1;
        wrong_digest[last] = format!("--spent-sha256={DIGEST}");
        assert!(parse_owned(wrong_digest).is_err());
    }
    ...
    #[test]
    fn ordering_diagnostic_rejects_tuning_and_other_modes() {
        for extra in [
            "--reference-threshold=0.80".to_string(),
            "--development-only".to_string(),
            "--sealed=sealed.csv".to_string(),
            ...
            let mut arguments = ordering_diagnostic_arguments();
            arguments.push(extra.clone());
            assert!(parse_owned(arguments).is_err(), "{extra}");
        }
    }
    ...
    #[test]
    fn capitalization_diagnostic_requires_exactly_v1_through_v5() {
        let arguments = parse_owned(capitalization_diagnostic_arguments()).unwrap();
        assert!(arguments.diagnose_capitalization_evidence);
        assert!(!arguments.diagnose_c5_calibration_frontier);
        assert_eq!(arguments.clean_csv, None);
        assert_eq!(arguments.spent_holdouts.len(), 5);

        let mut incomplete = capitalization_diagnostic_arguments();
        incomplete.truncate(incomplete.len() - 3);
        assert!(parse_owned(incomplete).is_err());

        let mut wrong_digest = capitalization_diagnostic_arguments();
        let last = wrong_digest.len() - 1;
        wrong_digest[last] = format!("--spent-sha256={DIGEST}");
        assert!(parse_owned(wrong_digest).is_err());
    }
    ...
    #[test]
    fn capitalization_diagnostic_rejects_tuning_and_other_modes() {
        for extra in [
            "--reference-threshold=0.80".to_string(),
            "--development-only".to_string(),
            "--sealed=sealed.csv".to_string(),
            "--diagnose-ordering-evidence".to_string(),
            "--diagnose-c5-calibration-frontier".to_string(),
        ] {
            let mut arguments = capitalization_diagnostic_arguments();
            arguments.push(extra.clone());
            assert!(parse_owned(arguments).is_err(), "{extra}");
        }
    }
    ...
    #[test]
    fn morphology_diagnostic_requires_totals_and_exactly_v1_through_v5() {
        let arguments = parse_owned(morphology_diagnostic_arguments()).unwrap();
        assert!(arguments.diagnose_morphology_evidence);
        assert!(!arguments.diagnose_c5_calibration_frontier);
        assert_eq!(
            arguments.morphology_name_totals,
            Some(PathBuf::from("name-totals.csv"))
        );
        assert_eq!(arguments.clean_csv, None);
        assert_eq!(arguments.spent_holdouts.len(), 5);

        let mut missing_totals = morphology_diagnostic_arguments();
        missing_totals.pop();
        assert!(parse_owned(missing_totals).is_err());

        let mut incomplete = morphology_diagnostic_arguments();
        incomplete.drain(incomplete.len() - 4..incomplete.len() - 1);
        assert!(parse_owned(incomplete).is_err());
    }

    #[test]
    fn morphology_totals_are_forbidden_outside_morphology_mode() {
        let mut arguments = calibration_frontier_arguments();
        arguments.push("--morphology-name-totals=name-totals.csv".to_string());
        assert!(parse_owned(arguments).is_err());

        let mut arguments = morphology_diagnostic_arguments();
        arguments.push("--diagnose-capitalization-evidence".to_string());
        assert!(parse_owned(arguments).is_err());
    }

    #[test]
    fn complement_diagnostic_requires_probes_and_exactly_v1_through_v5() {
        let arguments = parse_owned(complement_diagnostic_arguments()).unwrap();
        assert!(arguments.diagnose_complement_evidence);
        assert!(!arguments.diagnose_c5_calibration_frontier);
        assert_eq!(
            arguments.complement_probes,
            Some(PathBuf::from("probes.csv"))
        );
        assert_eq!(
            arguments.complement_surname_counts,
            Some(PathBuf::from("surname-counts.csv"))
        );
        assert_eq!(
            arguments.complement_surname_manifest,
            Some(PathBuf::from("surname-manifest.csv"))
        );
        assert_eq!(arguments.clean_csv, None);
        assert_eq!(arguments.spent_holdouts.len(), 5);

        let mut missing_probes = complement_diagnostic_arguments();
        missing_probes.retain(|argument| !argument.starts_with("--complement-probes="));
        assert!(parse_owned(missing_probes).is_err());

        let mut incomplete = complement_diagnostic_arguments();
        incomplete.drain(incomplete.len() - 6..incomplete.len() - 3);
        assert!(parse_owned(incomplete).is_err());
    }

    #[test]
    fn complement_probes_are_forbidden_outside_complement_mode() {
        let mut arguments = calibration_frontier_arguments();
        arguments.extend([
            "--complement-probes=probes.csv".to_string(),
            "--complement-surname-counts=surname-counts.csv".to_string(),
    ...
    #[test]
    fn surname_index_preparation_requires_exactly_v1_through_v7_and_private_probes() {
        let arguments = parse_owned(surname_index_arguments(true)).unwrap();
        assert!(arguments.prepare_surname_index_selection);
        assert_eq!(arguments.artifact, PathBuf::new());
        assert_eq!(arguments.spent_holdouts.len(), 7);
        assert_eq!(
            arguments.surname_selection_probes,
            Some(PathBuf::from("probes.csv"))
        );

        let mut incomplete = surname_index_arguments(true);
        incomplete.truncate(incomplete.len() - 3);
        assert!(parse_owned(incomplete).is_err());

        let mut missing_probes = surname_index_arguments(true);
        missing_probes.retain(|argument| !argument.starts_with("--surname-selection-probes="));
        assert!(parse_owned(missing_probes).is_err());
    }

    #[test]
    fn surname_index_selection_requires_fixed_inputs_and_exact_v1_through_v7() {
        let arguments = parse_owned(surname_index_arguments(false)).unwrap();
        assert!(arguments.select_surname_only_index);
        assert_eq!(arguments.spent_holdouts.len(), 7);
        assert_eq!(arguments.clean_csv, None);
        assert_eq!(
            arguments.surname_inventory,
            Some(PathBuf::from("inventory.csv"))
        );

        let mut missing_inventory = surname_index_arguments(false);
        missing_inventory.retain(|argument| !argument.starts_with("--surname-inventory="));
        assert!(parse_owned(missing_inventory).is_err());

        let mut wrong_digest = surname_index_arguments(false);
        let digest = wrong_digest
            .iter_mut()
            .find(|argument| argument.starts_with("--spent-sha256="))
            .unwrap();
    ...
    #[test]
    fn frozen_surname_candidate_verification_requires_only_fixed_artifact_inputs() {
        let arguments = parse_owned(surname_candidate_verification_arguments()).unwrap();
        assert!(arguments.verify_frozen_surname_candidate);
        assert_eq!(arguments.artifact, PathBuf::new());
        assert_eq!(arguments.clean_csv, None);
        assert_eq!(
            arguments.surname_member_keys,
            Some(PathBuf::from("members.txt"))
        );

        for prefix in [
            "--surname-membership-directory=",
            "--surname-member-keys=",
            "--surname-name-totals=",
        ] {
            let mut missing = surname_candidate_verification_arguments();
            missing.retain(|argument| !argument.starts_with(prefix));
            assert!(parse_owned(missing).is_err(), "{prefix}");
        }
        let mut forbidden = surname_candidate_verification_arguments();
        forbidden.push("--sealed=sealed.csv".to_string());
        forbidden.push("--sealed-manifest=manifest.csv".to_string());
        assert!(parse_owned(forbidden).is_err());
    }
    ...
    fn v8_validation_requires_pinned_digest_sealed_pair_and_verification_receipt() {
        ...
        assert_eq!(
            arguments
                .validate_frozen_surname_candidate_sha256
                .as_deref(),
            Some(surname_complement_validation::V8_SHA256)
        );
        assert_eq!(
            arguments.surname_verification_receipt,
            Some(PathBuf::from("artifact-validation.csv"))
        );

        for prefix in [
            "--surname-membership-directory=",
            "--surname-verification-receipt=",
            "--sealed=",
            "--sealed-manifest=",
        ] {
            let mut missing = v8_validation_arguments();
            missing.retain(|argument| !argument.starts_with(prefix));
            assert!(parse_owned(missing).is_err(), "{prefix}");
        }
        let mut wrong_digest = v8_validation_arguments();
        let digest = wrong_digest
            .iter_mut()
            .find(|argument| argument.starts_with("--validate-frozen-surname-candidate-sha256="))
            .unwrap();
...
#[cfg(test)]
mod production_c5_parity_tests {
    use super::*;
    ...
    #[test]
    fn production_c5_matches_benchmark_on_regression_dev_and_validation() {
        ...
        for case in cases {
            ...
            let expected_source = source_winner(&case.input, &diagnostic, &c5);
            let expected_emission = (c5.emission_source != C5EmissionSource::Abstain)
            ...
            assert_eq!(
                ...
                c5.c4.c31.final_score.to_bits(),
                "{}",
                case.id,
            );
            assert_eq!(
                production.inference.emission_source,
                public_emission_source(c5.emission_source),
                "{}",
                case.id,
            );
            assert_eq!(
            ...
            assert_decision_trace(&production.decision, &c5, &case.id);

            match c5.emission_source {
                C5EmissionSource::C31 => counts.c31 += 1,
                C5EmissionSource::SoleNative => counts.sole_native += 1,
                C5EmissionSource::DominantWinner => counts.dominant_winner += 1,
                C5EmissionSource::C5 => counts.c5 += 1,
                C5EmissionSource::Abstain => counts.abstain += 1,
            }
        }
    ...
    fn source_winner<'a>(
        ...
        c5.c4.c31.winner.as_ref()?;
        let candidate = diagnostic.candidates.first()?;
        input.get(candidate.byte_start?..candidate.byte_end?)
    }

    fn public_emission_source(source: C5EmissionSource) -> bonjour::EmissionSource {
        match source {
            C5EmissionSource::C31 => bonjour::EmissionSource::C31,
            C5EmissionSource::SoleNative => bonjour::EmissionSource::SoleNative,
            C5EmissionSource::DominantWinner => bonjour::EmissionSource::DominantWinner,
            C5EmissionSource::C5 => bonjour::EmissionSource::C5,
            C5EmissionSource::Abstain => bonjour::EmissionSource::Abstain,
        }
    }

    fn assert_decision_trace(
        public: &bonjour::DecisionTrace,
        internal: &classifier::C5DecisionBreakdown,
        case_id: &str,
    ) {
        assert_eq!(
        ...
        assert_rule_trace(&public.sole_native, &internal.c4.sole_native, case_id);
        assert_rule_trace(
            &public.dominant_winner,
            &internal.c4.dominant_winner,
            case_id,
        );
        assert_c5_rule_trace(&public.c5, &internal.controlled, case_id);
    }

    fn assert_c5_rule_trace(
        public: &bonjour::ControlledRuleTrace,
        internal: &classifier::C5RuleBreakdown,
        case_id: &str,
    ) {
        assert_eq!(public.c4_abstained, internal.c4_abstained, "{case_id}");
        assert_eq!(
            public.native_candidate, internal.native_candidate,
            "{case_id}"
        );
        assert_eq!(
            public.candidate_count, internal.candidate_count,
            "{case_id}"
        );
        assert_eq!(
        ...
        assert_eq!(
            public.winner_margin_min.map(f64::to_bits),
            internal.winner_margin_min.map(f64::to_bits),
            "{case_id}"
        );
        assert_eq!(
